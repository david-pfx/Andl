(function ($) {
	...
	$.fn.console = function (config) {
		...
		////////////////////////////////////////////////////////////////////////
		extern.writeMsg = function (msg) {
			var history = inner.find('.jquery-console-history-box')
			var message = $('<div>').addClass('jc-history-' + msg.type).html(msg.message);
			var rltn = inner.find('.jquery-console-content-box').data('prompt');
			if (prompt) {
				var box = history.find('#' + rltn)
				if (box) {
					box.after(message);
					return;
				}
		...
		function renderHistoryBox() {
			var box = $('<div class="jquery-console-history-box"></div>');
			inner.append(box);
		}

		function renderContentBox() {
			var box = $('<div class="jquery-console-content-box"></div>');
			inner.append(box);
		}
		...
		function appendToHistory() {
			var id = 'jqprompt' + (promptIndex++);
			updateClasses();
			appendPromptToHistory(id);
			setRelationPromptToContent(id);
		};

		function updateClasses() {
			var history = $('.jquery-console-history-box');
			history.find('.jquery-console-prompt-box')
				.removeClass('jquery-console-prompt-box')
				.addClass('jc-history-prompt-box');
		};

		function appendPromptToHistory(id) {
			var history = $('.jquery-console-history-box');
			var prompt = $('.jquery-console-prompt-box');
			prompt.attr('id', id).appendTo(history);
		};

		function setRelationPromptToContent(id) {
			var box = $('.jquery-console-content-box');
			box.data('prompt', id);
		};
		...
		function message(msg, className) {
			...
			$('.jquery-console-prompt-box').parent().append(mesg);
			mesg.show();
		};
